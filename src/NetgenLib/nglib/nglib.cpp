...
Ng_Meshing_Parameters :: Ng_Meshing_Parameters()
   ...
   DLL_HEADER Ng_OCC_Geometry * Ng_OCC_Load_Shape (TopoDS_Face& AShape)
   {
      OCCGeometry * occgeo = LoadOCC_Shape(AShape);
      return ((Ng_OCC_Geometry *)occgeo);
   }
   ...
   DLL_HEADER Ng_Result Ng_OCC_InitEdges(Ng_OCC_Geometry * geom,
		   Ng_Mesh * mesh, std::vector<double>& ANodes,
		   std::vector<std::vector<double> >& AEdges,
           Ng_Meshing_Parameters * mp)
   {
	   OCCGeometry * occgeom = (OCCGeometry*)geom;
      Mesh * me = (Mesh*)mesh;
      ...
      OCCFindEdges(*occgeom, *me, ANodes, AEdges);
      return NG_OK;
                                                    ...
                                                    Ng_Meshing_Parameters * mp)
   {
      ...
      std::cout<<"BUILD VISU"<<std::endl;
      occgeom->BuildFMap();
      occgeom->BuildVisualizationMesh(0.01);//ADD F. LEDOUX
   ...
   // Mesh the edges and add Face descriptors to prepare for surface meshing
   DLL_HEADER Ng_Result Ng_OCC_MyGenerateSurfaceMesh(TopoDS_Face& occ_face,
                                                    Ng_Mesh * mesh,
                                                    Ng_Meshing_Parameters * mp)
   {
	   ...
	   occgeo->BuildVisualizationMesh(0.01);
	   ...
	   OCCGenerateMesh( *occgeo, me,
			   netgen::MESHCONST_ANALYSE,
			   netgen::MESHCONST_OPTSURFACE, "ioio");
   ...
   void Ng_SurfaceMeshOrientation (Ng_Mesh * mesh)
   {
     Mesh * m = (Mesh*)mesh;
     m->SurfaceMeshOrientation();
   }
...
// F. LEDOUX definition presente dans le fichier meshing/msghandler.cpp
