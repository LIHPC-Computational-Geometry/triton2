...
//F. LEDOUX pour supprimer l'adhérence au Partition Spliter
...
namespace netgen
{
   ...
   void OCCGeometry :: HealGeometry ()
   {
      ...
      if (splitpartitions)
      {
         cout << "- running SALOME partition splitter NOT HANDLED IN TRITON" << endl;
//
//         TopExp_Explorer e2;
...
//            cout << " after : " << count << " solids" << endl;
   ...
   void OCCGeometry :: BuildVisualizationMesh (double deflection)
   {
      ...
      BRepMesh_IncrementalMesh sh(shape, deflection, true);
      sh.Perform();
      cout << "done" << endl;
   }
   ...
   void OCCGeometry :: WriteOCC_STL(char * filename)
   {
      ...
      //Commented after OCC 6.9.0 port
