...
#define IGNORECURVELENGTH 1e-4
   ...
   void FindNodesCorrespondingToEdge (
		   TopoDS_Edge & edge, //IN
		   std::vector<std::vector< Point<3> > >& curveNodes, //IN
		   Array<MeshPoint> & ps, //OUT
		   Array<double> & params, //OUT
		   Mesh & mesh)
   {
	   //we look for the set of points, which is the closest to the geometric curve of edge
	   double s0, s1; //s0 and s1 gives parameter range.
	   double maxh = mparam.maxh;
	   ...
	   Handle(Geom_Curve) c = BRep_Tool::Curve(edge, s0, s1);
	   std::vector<double> distancesToCurve;

	   for(unsigned int i=0;i<curveNodes.size();i++){
		   std::vector< Point<3> > points = curveNodes[i];
		   double dist = 0;
		   for(unsigned int j=0;j<points.size();j++){
			   Point<3> cp= points[j];
			   gp_Pnt pnt(cp(0),cp(1),cp(2));
			   GeomAPI_ProjectPointOnCurve proj(pnt,c);
			   proj.Perform(pnt);
			   if (proj.NbPoints()!=0)
				   dist +=proj.Distance(1);
		   }
		   distancesToCurve.push_back(dist);
	   }

	   double minDist = distancesToCurve[0];
	   int curve_id = 0;
	   for(unsigned int i=0;i<distancesToCurve.size();i++){
		   if(distancesToCurve[i]<minDist){
			   minDist = distancesToCurve[i];
			   curve_id = i;
		   }

	   }
	   cout<<"--------- min Dist : "<<minDist<<" on curve "<<curve_id<<endl;
	   std::vector< Point<3> > selected_points = curveNodes[curve_id];
	   int nsubedges = selected_points.size()-1;
	   ps.SetSize(nsubedges-1);
	   params.SetSize(nsubedges+1);

	   gp_Pnt curve_firstPoint = c->Value(s0);
	   Point<3> pfirst = selected_points[0];
	   int nbDiv = selected_points.size()-1;
	   std::cout<<"(s0,s1) = ("<<s0<<", "<<s1<<")"<<std::endl;
	   if(Dist2 (Point<3>(curve_firstPoint.X(), curve_firstPoint.Y(), curve_firstPoint.Z()),
			   pfirst) < 1e-5)
	   {
		   std::cout<<"############ 0 --> "<<s0<<std::endl;
		   params[0]=s0;
		   for (int i = 1; i < selected_points.size()-1; i++)
		   {
			   ps[i-1] = selected_points[i];
			   params[i] =s0+(i/double(nbDiv))*(s1-s0);
			   std::cout<<"############"<<i<<" --> "<<params[i]<<std::endl;
		   }
		   params[nsubedges] = s1;
		   std::cout<<"############ "<<nsubedges<<" --> "<<s1<<std::endl;
	   }
	   else
	   {
		   params[0]=s1;
		   std::cout<<"========== 0 --> "<<s1<<std::endl;
		   for (int i = nsubedges-1; i>0; i--)
		   {
			   ps[nsubedges-i-1]= selected_points[i];
			   params[nsubedges-i] = s1-(i/double(nbDiv))*(s1-s0);
			   std::cout<<"=========* "<<i<<" --> "<<params[nsubedges-i]<<std::endl;
		   }
		   params[nsubedges] = s0;
		   std::cout<<"========== "<<nsubedges<<" --> "<<s0<<std::endl;
	   }
   }
   ...
   void OCCFindEdges (OCCGeometry & geom, Mesh & mesh,
		   std::vector<double>& ACornerNodes,
		   std::vector<std::vector<double> >& ACurveNodes)
   {

	   cout << "==== edge meshing ====" << endl;

	   int nvertices = geom.vmap.Extent();
	   int nedges = geom.emap.Extent();
	   int nfaces = geom.fmap.Extent();

	   cout << "nb corner nodes  = " << ACornerNodes.size()/3 << endl;
	   cout << "nb geom vertices = " << nvertices << endl;
	   cout << "nb geom edges    = " << nedges << endl;
	   cout << "nb geom faces    = " << nfaces << endl;

	   if(nfaces!=1)
		   cout<<"ERROR: only one surface can be meshed!!"<<endl;
	   double eps = 1e-6 * geom.GetBoundingBox().Diam();

	   // We get nodes corresponding to each geometric vertices
	   // ordered in the order of geom edges (otherwise, surface
	   // meshing fails)
	   int nb_corner_assoc=0;
	   for (int i = 1; i <= nvertices; i++)
	   {
		   gp_Pnt pnt = BRep_Tool::Pnt (TopoDS::Vertex(geom.vmap(i)));
		   MeshPoint mp2( Point<3>(pnt.X(), pnt.Y(), pnt.Z()) );
		   bool found=false;
		   //for each corner, we check it corresponds to a geometric vertices of
		   // the OCCGeometry object geom
		   unsigned int i_corner=0;
		   for(;i_corner<ACornerNodes.size() && !found;){
			   MeshPoint mp( Point<3>(ACornerNodes[i_corner++],
					   ACornerNodes[i_corner++], ACornerNodes[i_corner++]) );

			   if ( Dist2 (Point<3>(mp2), Point<3>(mp)) < eps*eps){
				   found=true;
				   nb_corner_assoc++;
				   std::cout<<"Corner ("<<pnt.X()<<","<<pnt.Y()<<", "<<pnt.Z()<<")"<<std::endl;
			   }
		   }
		   // the corner point is added
		   if (found)
			   mesh.AddPoint (mp2);
		   else {
			   cout<<"ERROR: A given corner is not associated to a geometric vertex"<<endl;
			   std::cout<<"Current point ("<<pnt.X()<<","<<pnt.Y()<<", "<<pnt.Z()<<")"<<std::endl;
			   unsigned int i_corner2=0;
			   for(;i_corner2<ACornerNodes.size() && !found;){
				   std::cout<<"Corner ("
						   <<ACornerNodes[i_corner2++]<<","
						   <<ACornerNodes[i_corner2++]<<", "
						   <<ACornerNodes[i_corner2++]<<")"<<std::endl;
			   }
			   exit(0);
		   }
	   }

	   if(nb_corner_assoc!=ACornerNodes.size()/3){
		   cout<<"ERROR: Too many associations to geometric vertices"<<endl;
	   }
	   cout << "nb mesh nodes created (one per geom vertices) = " << nb_corner_assoc<<endl;
	   //mesh.GetNP() << endl;

	   // Creation of point<3> vector to associate  nodes to geometric curves
	   std::vector<std::vector< Point<3> > > curve_points;
	   for(unsigned int i=0;i<ACurveNodes.size();i++){
	   		std::vector<double> edge_nodes = ACurveNodes[i];
	   		std::vector< Point<3> > points;
	   		for (unsigned int j=0;j<edge_nodes.size();){
	   			points.push_back(Point<3>(edge_nodes[j++], edge_nodes[j++], edge_nodes[j++]));
	   		}
	   ...
	   int edgenr = 0;
	   int curr = 0;
	   TopoDS_Face face = TopoDS::Face(geom.fmap(1));
	   facenr = geom.fmap.FindIndex (face);

	   int solidnr0 = 0;
	   int solidnr1 = 0;

	   cout << "\t Face info : " << facenr<<" - "<<solidnr0<<" "<<solidnr1<<endl;
	   ...
	   for (TopExp_Explorer exp2 (face, TopAbs_WIRE); exp2.More(); exp2.Next())
	   {
		   ...
		   {
			   curr++;
			   cout << "edge nr " << curr << endl;

			   TopoDS_Edge edge = TopoDS::Edge (exp3.Current());
			   if (BRep_Tool::Degenerated(edge))
			   {
				   cout << "ignoring degenerated edge" << endl;
				   continue;
			   }
			   ...
			   //tableau a remplir de points avec ceux passe en parametres
			   // mp ne contient que les points internes de la courbe
			   Array <MeshPoint> mp;
			   // parametres des points internes + les points extremites en 0 et back()
               Array <double> params;

			   FindNodesCorrespondingToEdge(edge, curve_points, mp, params,mesh);
			   ...
			   cout << "First point " << mp[0] <<", curve param "<<params[0]<<endl;
			   cout << "Last  point " << mp[mp.Size()-1] <<", curve param "<<params[params.Size()-2]<<endl;
			   cout << "between params ("<<params[0]<<", "<<params[params.Size()-1]<<")"<<endl;

			   for (int i = 0; i < mp.Size(); i++)
			   {
				   MeshPoint pi = mp[i];
				   mesh.AddPoint (pi);
				   pnums[i+1] = mesh.GetNP();
				   cout << "add meshpoint " << pi <<", curve param "<<params[i+1]<<" num: "<<pnums[i+1]<<endl;

			   }

			   cout << "Number of mesh nodes = " << mesh.GetNP() << endl;
			   ...
			   for (int i = 1; i <= mp.Size()+1; i++)
			   {
				   ...
				   gp_Pnt2d p2d = cof->Value(params[i-1]);
				   ...
				   std::cout<<"Add segment "<<seg.edgenr<<" --> "<<seg[0] <<" - "<<seg[1]<<" on "<<endl;
				   std::cout<<"\t "<<seg.si<<endl;
				   std::cout<<"\t "<<seg.epgeominfo[0].dist<<" "<<seg.epgeominfo[1].dist<<endl;
				   std::cout<<"\t "<<seg.epgeominfo[0].edgenr<<" "<<seg.epgeominfo[1].edgenr<<endl;
				   std::cout<<"\t "<<seg.epgeominfo[0].u<<" - "<<seg.epgeominfo[0].v<<endl;
				   std::cout<<"\t "<<seg.epgeominfo[1].u<<" - "<<seg.epgeominfo[1].v<<endl;
